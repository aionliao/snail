...
func (self *array)read(index int)(packet av.Packet,nextindex int,err error){
	//log.Println("indx:",index,self.index,self.timeID,len(self.packets))
	if index > self.index{
...
func (self *GopCache)Read(pos int, curid int64)(packet av.Packet,nextpos int,id int64,err error){
	...
	if pos == -1{
		...
		log.Println("ssss",index)
	}else{
			index = pos >> 16
			indexpos = pos & 0xffff
			log.Println("1:",index, indexpos)
